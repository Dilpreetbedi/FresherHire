...
export default function CandidatesPage() {
  ...
  async function loadCandidates() {
    ...
    try {
      ...
      if (profileError) {
        console.error("Recruiter profiles error:", profileError);

        setError(profileError.message || "Could not load candidates.");
      ...
      if (shortlistsResult.error) {
        console.error("Shortlist error:", shortlistsResult.error);
      }

      const loadedSkills = skillsResult.data || [];

      const loadedAssessments = assessmentsResult.data || [];

      const loadedProjects = projectsResult.data || [];

      setShortlistedIds(
        (shortlistsResult.data || []).map((item) => item.candidate_id),
      );
      ...
      loadedAssessments.forEach((assessment) => {
        const existing = assessmentsByUser.get(assessment.user_id) || [];

        existing.push(assessment);

        assessmentsByUser.set(assessment.user_id, existing);
      });
      ...
      const formatted = (profiles || []).map((profile) => ({
        ...
        assessments: assessmentsByUser.get(profile.id) || [],
  ...
  function getBestAssessments(candidate: CandidateWithData) {
  ...
  function getBestScore(candidate: CandidateWithData) {
    const bestAssessments = getBestAssessments(candidate);

    if (bestAssessments.length === 0) {
      return 0;
    }

    return Math.max(
      ...bestAssessments.map((assessment) => assessment.percentage),
    );
  }

  function hasVerifiedAssessment(candidate: CandidateWithData) {
    return getBestAssessments(candidate).some(
      (assessment) => assessment.percentage >= VERIFIED_THRESHOLD,
    );
  }
  ...
  function getAssessmentResult(
    ...
    return getBestAssessments(candidate).find(
      (assessment) =>
        normalizeValue(assessment.skill_name) ===
        normalizeValue(assessmentName),
    );
  }

  const availableSkills = useMemo(() => {
    const values = candidates.flatMap((candidate) =>
      candidate.skills.map((skill) => skill.skill_name),
    );
  ...
  const availableAssessments = useMemo(() => {
    const values = candidates.flatMap((candidate) =>
      getBestAssessments(candidate).map((assessment) => assessment.skill_name),
    );
  ...
  const availableRoles = useMemo(() => {
    const values = candidates
      .map((candidate) => candidate.preferred_role)
  ...
  const availableGraduationYears = useMemo(() => {
    const values = candidates
      .map((candidate) => candidate.graduation_year)
    ...
    return candidates
      .filter((candidate) => {
        ...
        const searchMatch =
          ...
          candidate.preferred_role?.toLowerCase().includes(searchText) ||
          candidate.location?.toLowerCase().includes(searchText) ||
          candidate.degree?.toLowerCase().includes(searchText) ||
          candidate.skills.some((skill) =>
            skill.skill_name.toLowerCase().includes(searchText),
          ) ||
          bestAssessments.some((assessment) =>
            assessment.skill_name.toLowerCase().includes(searchText),
          );
        ...
        const skillMatch =
          ...
          candidate.skills.some(
            (skill) =>
              normalizeValue(skill.skill_name) === normalizeValue(skillFilter),
          );
        ...
        const assessmentMatch =
          ...
          bestAssessments.some(
            (assessment) =>
              normalizeValue(assessment.skill_name) ===
              normalizeValue(assessmentFilter),
          );

        const verifiedMatch =
          !verifiedOnly ||
          (assessmentFilter !== "All"
            ? (getAssessmentResult(candidate, assessmentFilter)?.percentage ??
                0) >= VERIFIED_THRESHOLD
            : hasVerifiedAssessment(candidate));
        const roleMatch =
          roleFilter === "All" || candidate.preferred_role === roleFilter;
        ...
        const graduationMatch =
          ...
          String(candidate.graduation_year) === graduationFilter;

        const bestScore = getBestScore(candidate);

        const scoreMatch = bestScore >= Number(scoreFilter);

        const resumeMatch = !resumeOnly || candidate.has_resume;

        const projectMatch = !projectsOnly || candidate.projects.length > 0;
  ...
  const activeFilterCount = [
    ...
    assessmentFilter !== "All" ? assessmentFilter : "",
    roleFilter !== "All" ? roleFilter : "",
    locationFilter !== "All" ? locationFilter : "",
    graduationFilter !== "All" ? graduationFilter : "",
    scoreFilter !== "0" ? scoreFilter : "",
  ...
  async function shortlistCandidate(candidateId: string) {
    if (shortlistingId) {
      return;
    }
    ...
    try {
      ...
      const { data: insertedShortlist, error: shortlistError } = await supabase
        .from("shortlists")
        .insert({
          company_id: user.id,
      ...
      if (shortlistError) {
        if (shortlistError.code === "23505") {
          ...
          setActionError(
            shortlistError.message || "Could not shortlist candidate.",
          );
        }
      ...
      setShortlistedIds((current) => [...new Set([...current, candidateId])]);
    } catch (err) {
      console.error("Shortlist candidate error:", err);

      setActionError(
        err instanceof Error ? err.message : "Could not shortlist candidate.",
      );
    }
  ...
  async function removeShortlist(candidateId: string) {
    if (shortlistingId) {
      return;
    }
    ...
    try {
      ...
      const { data: deletedShortlist, error: removeError } = await supabase
        .from("shortlists")
        .delete()
        .eq("company_id", user.id)
      ...
      if (removeError) {
        ...
        setActionError(
          removeError.message || "Could not remove candidate from shortlist.",
        );
      ...
      if (!deletedShortlist) {
        setActionError("Candidate was not found in your shortlist.");
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
        <div>
          ...
          <p className="mt-3 max-w-3xl leading-7 text-slate-600">
            Discover entry-level candidates across technical and non-technical
            roles using skills, verified assessments, work samples, education
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              >
                ...
                <option value="75">75%+ Verified</option>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <FilterToggle
              active={resumeOnly}
              onClick={() => setResumeOnly(!resumeOnly)}
              label="📄 Has Resume"
            />

            <FilterToggle
              active={projectsOnly}
              onClick={() => setProjectsOnly(!projectsOnly)}
        ...
        <div className="mt-8 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-slate-950">Candidates</h2>
        ...
        {!loading && filteredCandidates.length > 0 && (
          <div className="mt-6 grid gap-5">
            {filteredCandidates.map((candidate) => {
              ...
              const verifiedCount = bestAssessments.filter(
                (assessment) => assessment.percentage >= VERIFIED_THRESHOLD,
              ).length;

              const shortlisted = shortlistedIds.includes(candidate.id);
                ...
                >
                  <div className="flex flex-col justify-between gap-6 md:flex-row">
                    <div className="flex gap-5">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-sm font-medium text-blue-600">
                          {candidate.preferred_role || "Entry-Level Candidate"}
                        </p>
                        co
                        <p className="mt-2 text-sm text-slate-500">
                          {candidate.degree || "Qualification not provided"}
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          ...
                          {candidate.projects.length > 0 && (
                            <span className="rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-xs font-medium text-violet-700">
                              📁 {candidate.projects.length} Work Sample
                          ...
                          {verifiedCount > 0 && (
                            <span className="rounded-full border border-green-200 bg-green-50 px-3 py-1 text-xs font-medium text-green-700">
                              ✓ {verifiedCount} Verified Assessment
                              {verifiedCount !== 1 ? "s" : ""}
                            </span>
                          )}
                        </div>
                    ...
                    <div className="h-fit rounded-xl border border-slate-200 bg-slate-50 px-6 py-4 text-center">
                      ...
                      >
                        {bestAssessments.length > 0 ? `${bestScore}%` : "—"}
                      </p>
                    ...
                    <div className="mt-5">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {bestAssessments.map((assessment) => {
                          const verified =
                            assessment.percentage >= VERIFIED_THRESHOLD;
                            ...
                            >
                              ...
                              <span
                                className={`ml-2 font-semibold ${
                                  verified ? "text-green-700" : "text-slate-600"
                                }`}
                              >
